...
async fn passthrough_duplicate_returns_false() {
    ...
    let r2 = log_passthrough_event(&pool, Some("pi_ptd"), "evt_ptd_1", "charge.created", 1000, &payload)
        .await
        .unwrap();
    assert!(!r2); // duplicate
}
...
async fn passthrough_links_existing_payment() {
    ...
    log_passthrough_event(&pool, Some("pi_ptlink"), "evt_ptlink_pt", "charge.succeeded", 2000, &payload)
        .await
        .unwrap();
...
async fn passthrough_no_existing_payment() {
    ...
    let row: Option<(Option<uuid::Uuid>,)> = sqlx::query_as(
        "SELECT entity_id FROM audit_log WHERE event_id = $1",
    )
    .bind("evt_ptnone")
    .fetch_optional(&pool)
    .await
...
async fn passthrough_with_none_external_id() {
    ...
    let row: Option<(Option<String>, Option<uuid::Uuid>)> = sqlx::query_as(
        "SELECT external_id, entity_id FROM audit_log WHERE event_id = $1",
    )
    .bind("evt_ptnull")
    .fetch_optional(&pool)
    .await
